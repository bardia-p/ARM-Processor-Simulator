...
; R0 = indexed character with higher bits cleared
  ...
  POP {R1,R2,R3,R4, R15 }    ; restore registers and return 


; uint LengthOf( &( PackedString[] ) )
; Finds the length of a packed string
LengthOf  
 PUSH {R1, R2,R14} 
...
TestFor
  ...
  ; get address of ExampleString
    MOV  R0, R5  ; Prepare R0 (i.e. the first parameter of CharAt() )
  ; get character index 
...
; R0 will save the length of the string 
   MOV R0, R2
   POP {R1, R2, R15} 
...
; at this point, R2 contains the number of characters in ExampleString
   ; get address of ExampleString
   LEA  R5, [ ExampleString ]  ; Prepare R5 (i.e. the first parameter of CharAt() )
